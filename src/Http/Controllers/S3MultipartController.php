...
class S3MultipartController extends Controller
{
    ...
    {
        $this->disk = config('filament-s3-multipart-upload.disk');
     $this->bucket= config("filesystems.disks.$this->disk.bucket");
    ...
    public function createMultipartUpload(Request $request)
    {
        ...
        $key = Str::of($request->input('filename'))->replaceStart('/','')->slug();
    ...
    public function getSignedUrl(Request $request, int $partNumber)
    {
        ...
        $result = $this->client->createPresignedRequest($command, config('filament-s3-multipart-upload.expiry'));
